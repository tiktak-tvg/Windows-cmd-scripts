...
Sub LogCopy(SourceFolder, DestinationFolder)
    ...
    '/// Если конечная папка не существует - то создаём её
    'If Not FileSystemObject.FolderExists(DestinationFolder) Then
        'FileSystemObject.CreateFolder DestinationFolder
    
        '/// Если произошла ошибка, то пишем её в лог и выходим из процедуры, т.к дальнейшее копирование в эту папку невозможно
        'If Err.Number <> 0 Then
            'WriteLog "Не удалось создать конечный каталог " & DestinationFolder & " " & Err.Number & " " & Err.Description, 1
           ' Exit Sub
        'Else
            'WriteLog "Копируется папка " & DestinationFolder, 0
        'End If
    'End If

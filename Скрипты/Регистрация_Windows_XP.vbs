...
WshShell.Run "%SystemRoot%\system32\regsvr32.exe EasyGrid.ocx ", 0, false
WScript.Sleep 1700 
WshShell.Run "%SystemRoot%\system32\regsvr32.exe vfp9r.dll ", 0, false
WScript.Sleep 1700 
WshShell.Run "%SystemRoot%\system32\regsvr32.exe vfp9t.dll ", 0, false
WScript.Sleep 1700 
WshShell.Run "%SystemRoot%\system32\regsvr32.exe Richtx32.ocx ", 0, false
WScript.Sleep 1700 
WshShell.Run "%SystemRoot%\system32\regsvr32.exe mscomct2.ocx ", 0, false
WScript.Sleep 1700 
WshShell.Run "%SystemRoot%\system32\regsvr32.exe MSCOMCTL.OCX ", 0, false
WScript.Sleep 1700 
WshShell.Run "%SystemRoot%\system32\regsvr32.exe MSCOMM32.OCX ", 0, false
WScript.Sleep 2000
	intReturn = WshShell.Popup("Программа регистрации библиотек программы ""Запись на приём"" на Windows XP завершила работу и автоматически закроется через 5 секунд. Нажмите ОК, чтобы закрыть. Нажмите Отмена, чтобы не закрывать", 10, "Внимание", vbOkCancel + vbQuestion)
	If (intReturn = vbCancel) Then
		Set WshShell = Nothing
		Wscript.Quit 1
	Else
		WshShell.Run "%SystemRoot%\system32\cmd.exe /C taskkill /f /im iexplore.exe /im regsvr32.exe", 0
	End If
